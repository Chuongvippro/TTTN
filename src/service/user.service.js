...
class userService {
  ...
  async restoreUser(id) {
    ...
    // Ví dụ: Check xem user có thực sự đang bị banned không mới cho restore
    const user = await this.userRepo.findUserByIdForAdmin(id);
    if (!user) throw new Error("Không tìm thấy người dùng");

    return await this.userRepo.restoreStatus(id);
  }
